...
/** Android 视频解码器选择模式。 */
enum class PlatformVideoDecoderMode(val raw: Int) {
  HARDWARE_PREFERRED(0),
  SOFTWARE_ONLY(1);

  companion object {
    fun ofRaw(raw: Int): PlatformVideoDecoderMode? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
...
data class CreationOptions (
  ...
  val decoderMode: PlatformVideoDecoderMode
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): CreationOptions {
      ...
      val decoderMode = pigeonVar_list[4] as PlatformVideoDecoderMode
      return CreationOptions(uri, formatHint, httpHeaders, userAgent, decoderMode)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
private open class MessagesPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      130.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          PlatformVideoDecoderMode.ofRaw(it.toInt())
        }
      }
      131.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
